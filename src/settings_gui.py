...
class SettingsWindow:
    ...
    def _setup_ui(self):
        ...
        ttk.Entry(main_frame, textvariable=self.hotkey_var).grid(row=0, column=1, sticky=tk.EW, pady=5)
        ...
        ttk.Combobox(main_frame, textvariable=self.model_var, values=models, state="readonly").grid(row=1, column=1, sticky=tk.EW, pady=5)
        ...
        devices = ["cuda", "cpu"]
        ttk.Combobox(main_frame, textvariable=self.device_var, values=devices, state="readonly").grid(row=2, column=1, sticky=tk.EW, pady=5)
        
        # Language
        ttk.Label(main_frame, text="Language (null for auto):").grid(row=3, column=0, sticky=tk.W, pady=5)
        self.lang_var = tk.StringVar(value=str(self.config.language) if self.config.language else "")
        ttk.Entry(main_frame, textvariable=self.lang_var).grid(row=3, column=1, sticky=tk.EW, pady=5)
        
        # Notifications
        self.notify_var = tk.BooleanVar(value=self.config.enable_notifications)
        ttk.Checkbutton(main_frame, text="Enable Notifications", variable=self.notify_var).grid(row=4, column=0, columnspan=2, sticky=tk.W, pady=5)
        
        # Logging
        self.logging_var = tk.BooleanVar(value=self.config.get("enable_logging", True))
        ttk.Checkbutton(main_frame, text="Enable Training Data Logging", variable=self.logging_var).grid(row=5, column=0, columnspan=2, sticky=tk.W, pady=5)
        
        # Auto-start
        self.autostart_var = tk.BooleanVar(value=self.config.start_with_windows)
        ttk.Checkbutton(main_frame, text="Start with Windows", variable=self.autostart_var).grid(row=6, column=0, columnspan=2, sticky=tk.W, pady=5)
    ...
    def _save(self):
        ...
        messagebox.showinfo("Murmur", "Settings saved! Some changes may require a restart.")
        self.root.destroy()
